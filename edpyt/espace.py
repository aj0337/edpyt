...
from edpyt.sector import binom, generate_states, get_sector_dim

SzStates = namedtuple("States", ["up", "dw"])
Sector = make_dataclass(
    "Sector",
    [
        "states",
        ("d", int),
...
def build_empty_sector(n, *p):
    ...
    if len(p) == 1:
        ...
        nup, ndw = p
        states = SzStates(generate_states(n, nup), generate_states(n, ndw))
...
def build_non_interacting_espace(ek):
    ...
    for qns, sct in _sz_iter_sectors(n):
        ...
        add_onsites(ek, Uk, states.up, states.dw, eigvals, 0.0)
        eigvals.sort()
        sct.eigvals = eigvals
        espace[qns] = sct
...
def screen_espace(espace, egs, beta=1e6, cutoff=1e-9):
    ...
    for qns, sct in espace.items():
        diff = np.exp(-beta * (sct.eigvals - egs))
        keep_mask = diff > cutoff
        if np.any(keep_mask):
            sct.eigvals = sct.eigvals[keep_mask]
            sct.eigvecs = sct.eigvecs[:, keep_mask]
        else:
            delete.append(qns)
    for qns in delete:
        espace.pop(qns)
...
def adjust_neigsector(espace, neig, n):
    ...
    def _increase(isct):
        neig[isct] = min(get_sector_dim(n, nup, ndw), neig[isct] + 1)

    def _decrease(isct):
        neig[isct] = max(1, neig[isct] - 1)

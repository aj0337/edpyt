...
def _get_occps_method(comm):
    if comm is not None:
        ...
        def wrap(self, mu):
            occps_loc = integrate_gf(self, mu, self.nmats, self.beta)
        ...
        def wrap(self, mu):
            occps = integrate_gf(self, mu, self.nmats, self.beta)
...
class Gfloc:
    ...
    def __init__(self, H, S, Hybrid, idx_neq, idx_inv, nmats=3000, beta=70., comm=None) -> None:
        self.n = H.shape[-1]
        self.H = H
        self.S = S
        self.Hybrid = Hybrid
    ...
    def __call__(self, z):
        ...
        # G_inv = np.zeros((len(z), self.n, self.n), dtype=complex)
        # for i, z_ in enumerate(z):
        #     G_inv[i] = self.free(z_, inverse=True)
        #     # G_inv[i].flat[:: (self.n + 1)] -= sigma.T[i][self.idx_inv]
    ...
    def Delta(self, z):
        ...
        z = np.atleast_1d(z)
        weiss = self.Weiss(z)
        # delta = np.zeros((len(z), self.n), dtype=complex)
        # for i, z_ in enumerate(z):
        #     delta[i] = z_ + self.mu - self.ed - weiss[i]
        # return delta.T
    ...
    def Weiss(self, z):
        ...
        #  0,ii                     ii
        gloc = self(z)
        return np.reciprocal(gloc) + self.Sigma(z)
        gloc_inv = np.linalg.inv(gloc)
        return gloc_inv.diagonal(0, 1, 2)  # + self.Sigma(z)
